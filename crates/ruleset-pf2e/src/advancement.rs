...
//! append it while no card ever renders it. The choice slots a level opens
//! are registered by their own kind modules (feats, skills, spells),
//! keyed on `state.level()`.
...
use crate::mechanics::{sel_single, slot_level_advance, step_level_advance, Pf2eState};
        ...
        regs.push(SlotRegistration::<Pf2eState> {
            ...
            unlock: Box::new(move |state| {
                let cap = state
                    ...
                    .map(|c| c.level_cap())
                    .unwrap_or(1);
                if state.level() as u32 + 1 == level && level <= cap {
                    Availability::Open
                } else {
                    Availability::Hidden
                }
            ...
            apply: Box::new(move |state, decision| {
                ...
                for feature in &adv.features {
                    state.effects.extend(feature.effects.iter().cloned());
                }
                Ok(())
            }),

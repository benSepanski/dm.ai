...
// ---- chargen-wizard: prep-save and cascade cycles ----

#[path = "wizard_fixture.rs"]
mod wizard_fixture;

/// SIGKILL during prep saves (draft and finalized) and during the
/// school-change cascade: every file stays loadable; prep is always one
/// complete choice set (old or new, never torn); and the cascade commits
/// atomically — never a changed school with surviving curriculum-derived
/// preparation.
#[test]
fn prep_writes_under_sigkill_are_none_or_all() {
    let dir = tempfile::tempdir().unwrap();
    let client = reqwest::blocking::Client::builder()
        .timeout(std::time::Duration::from_secs(2))
        .build()
    ...
    // Build Sylvenne (draft, with initial prep) in a clean run.
    let built;
    {
        let server = TestServer::spawn(dir.path());
        built = wizard_fixture::build_sylvenne_draft(&client, &server.url);
    }
    let id = built.id.clone();

    let assert_intact = |server: &TestServer| -> Value {
        let roster: Value = client
            .get(format!("{}/api/roster", server.url))
            .send()
            .unwrap()
        ...
        assert!(
            roster["problems"].as_array().unwrap().is_empty(),
            "no torn files after a prep-cycle kill: {:?}",
            roster["problems"]
        );
        let doc: Value = serde_json::from_str(
            &std::fs::read_to_string(dir.path().join(format!("characters/{id}.json"))).unwrap(),
        )
        .unwrap();
        // Prep, when present, is a complete parseable set — never torn.
        if !doc["prep"].is_null() {
            assert!(
                doc["prep"]["choices"].is_array(),
                "prep section parses after a kill: {:?}",
                doc["prep"]
            );
        }
        doc
    ...
    // Cycle 1+2: kill during draft prep re-saves at varying offsets.
    for (cycle, delay_ms) in [1u64, 6].into_iter().enumerate() {
        let mut server = TestServer::spawn(dir.path());
        ...
        let fire = std::thread::spawn({
            ...
            move || {
                let _ = client
                    .post(format!("{url}/api/characters/{id}/prep"))
                    .json(&serde_json::json!({
                        "request_id": format!("crash-prep-{cycle}"),
                        "version": version,
                        "expected_state": "draft",
                        "choices": wizard_fixture::sylvenne_prep_choices(),
                    }))
                    .send();
            }
        });
        ...
        fire.join().unwrap();
        let server = TestServer::spawn(dir.path());
        assert_intact(&server);
    }

    // Cycle 3: kill during the school-change cascade (amend). Afterward the
    // file holds either the old school with its full prep, or the new
    // school with every curriculum-derived choice cleared — never a mix.
    {
        let mut server = TestServer::spawn(dir.path());
        let character: Value = client
            .get(format!("{}/api/characters/{id}", server.url))
            ...
            move || {
                let _ = client
                    ...
                    .json(&serde_json::json!({
                        "version": version,
                        "decision": {
                            "id": "crash-school-swap",
                            "slot": "pf2e.class.school",
                            "selection": {"kind": "option", "value": "school.protean-form"},
                            "source": "player"
                        }
                    }))
                    .send();
        ...
        fire.join().unwrap();
        let server = TestServer::spawn(dir.path());
        let doc = assert_intact(&server);
        let school = doc["log"]
            .as_array()
            .unwrap()
            .iter()
            .find(|d| d["slot"] == "pf2e.class.school")
            .map(|d| d["selection"]["value"].as_str().unwrap().to_string());
        let prep_slots: Vec<String> = doc["prep"]["choices"]
            .as_array()
            .map(|a| {
                a.iter()
                    .map(|c| c["slot"].as_str().unwrap().to_string())
                    .collect()
            })
            .unwrap_or_default();
        match school.as_deref() {
            Some("school.battle-magic") => {
                assert_eq!(prep_slots.len(), 4, "old state keeps the full prep");
            }
            Some("school.protean-form") => {
                assert_eq!(
                    prep_slots,
                    vec!["pf2e.prep.cantrips"],
                    "the cascade cleared every curriculum-derived choice \
                     in the same durable write"
                );
                // The curriculum spellbook additions cleared with it.
                assert!(
                    !doc["log"]
                        .as_array()
                        .unwrap()
                        .iter()
                        .any(|d| d["slot"] == "pf2e.class.spellbook.curriculum"),
                    "no stale curriculum additions survive a school change"
                );
            }
            None => {
                // The amend cleared but never landed? Impossible: amend is
                // one engine step + one write. School always exists.
                panic!("school slot vanished from the log");
            }
            other => panic!("unexpected school {other:?}"),
        }
    }
}

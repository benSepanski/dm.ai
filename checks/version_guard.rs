...
/// chargen-wizard: prep saves respect the version guard — a character
/// (draft or finalized) pinned to an older known version rejects prep
/// saves until resolution, with the flag attached and nothing written.
#[test]
fn prep_saves_on_older_pinned_characters_are_rejected() {
    let dir = tempfile::tempdir().unwrap();
    let client = reqwest::blocking::Client::new();
    let (id, _) = build_fixture(dir.path(), "OldPrep");
    pin_to_test_version(dir.path(), &id, true);
    let bytes_before = std::fs::read(character_path(dir.path(), &id)).unwrap();
    ...
    let response = client
        .post(format!("{}/api/characters/{id}/prep", server.url))
        .json(&json!({
            "request_id": "flagged-prep",
            "version": 2,
    ...
    assert_eq!(
        ...
        409,
        "a flagged character rejects prep saves with the flag attached"
    );
    let body: Value = response.json().unwrap();
    assert!(
        body["status"].is_object(),
        "the refusal carries the version status: {body:#?}"
    );
    assert_eq!(
        std::fs::read(character_path(dir.path(), &id)).unwrap(),
        bytes_before,

...
| **C** ✅ done — Weapon-registry ↔ attack-resolution bridge (`WeaponData` → `AttackDetails`, incl. dm-api) | EQP-01 ✅ done, EQP-08 ✅ done (Heavy/Versatile/Finesse wired at C; Ammunition/Loading wired post-D3, once inventory existed to back them), ACT-18 ✅ done | L |
...
| Work | Findings | Size |
|------|----------|------|
| **J (remainder)** — SpellData vocabulary: non-damage effects, multi-beam, on-hit riders, staged/choice conditions, divided healing, HP-max buffs, thresholds; SPL-08 ✅ done (spell-attack adv/disadv + crits) | SPL-08 ✅, SPL-09, SPL-10, SPL-11, SPL-12, SPL-13, SPL-14, SPL-18, SPL-20, SPL-21 | L |
...
- `WeaponProperty` enum & `AttackDetails.properties` have zero consumers: Heavy/Loading/Ammunition/Versatile/Finesse/Reach/Thrown unimplemented (`sheets.py:291` [EQP-08], **major**) ✅ done — Heavy, Versatile, Finesse have consumers since C; **Ammunition/Loading now do too** (landed after D3 gave the engine an inventory to back them): `WeaponData.ammunition_name` (`data/weapons.py`) maps each of the 9 Ammunition weapons to its `data/gear.py` ammo item, `_weapon_bridge.to_attack_details` copies it onto `AttackDetails`, `_attacks._validate_attack`/`_resolve_attack` gate on and spend one matching inventory unit per attack roll (hit or miss — `out_of_ammunition` fails the attack before any economy slot is spent), and `_actions._resolve_attack_action` caps a Loading weapon to one main-hand Attack-action shot per turn via `TurnState.loading_attack_used` (a separate bonus-action/reaction use, e.g. a paired Hand Crossbow's Nick shot, is its own "use" and isn't gated by this same flag). Reach/Thrown are positional/targeting concerns out of this engine's theater-of-mind scope and remain out of scope.
...
- Spell attack rolls ignore all condition-based advantage/disadvantage and never deal critical damage (`_spell_resolution.py:129` [SPL-08], **major**) ✅ done — a new shared `_advantage.py::_base_advantage_state` (factored out of `_attacks.py::_advantage_state`, which now calls it) aggregates condition-based advantage/disadvantage, Dodge, and the one-shot Help/Vex/Sap/Hide turn-state flags for both weapon and spell attack rolls. `cast_spell`'s `attack_roll` branch now calls it (treating `SpellRangeType.TOUCH` as melee, everything else as ranged), rolls with `roll_with_advantage`/`roll_with_disadvantage` accordingly, and sets `critical=True` on a natural 20 or a melee touch-range hit against a Paralyzed/Unconscious target — `_roll_damage` doubles the already-scaled damage dice (cantrip/upcast included) on a crit, and `_apply_damage_effective` is passed `critical=True` so the existing two-death-save-failure-on-crit logic (EFF-09) applies to spells too. `SpellTargetOutcome.critical` surfaces the flag to callers (dm-api's `cast-spell` combat-log entry includes it). See `tests/test_spellcasting_effects.py::TestSpellAttackAdvantageAndCrits`.
...
3. **Workstream C** ✅ done (weapon registry ↔ resolver bridge) — masteries/proficiency/Heavy reach the real dm-api pipeline; Ammunition/Loading tracking (EQP-08) landed after Workstream D3 gave the engine an inventory to back it.

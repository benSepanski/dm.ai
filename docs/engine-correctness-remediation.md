...
| **B3** ✅ SPL-03/SPL-06 done — Spell casting-time → economy slot; 2024 one-slot-spell-per-turn. TWF Light validation (ACT-04) remains, blocked on Workstream C | SPL-03, SPL-06, ACT-04 | M |
...
**SPL-03/SPL-06 (spell casting-time economy + one-slot-per-turn):** `spellcasting.check_casting_economy`/`commit_casting_economy` (`game-engine/src/game_engine/rules/dnd_5_5e/spellcasting.py`) are called directly from `_spell_resolution.cast_spell`, not from `_actions.py` — the generic `ActionType.MAGIC` branch in `_resolve_action_impl` has no `SpellData` to reason about (the `Action`/`ActionResult` interface never carries one), and the real pipeline (dm-api's `/combat/cast-spell` endpoint) always calls `cast_spell` directly rather than routing through `resolve_action`. Putting the gate inside `cast_spell` itself means it applies to *every* caller, present or future, per harness-engineering principle #1. `check_casting_economy` validates (read-only) that the spell's `casting_time` names an unspent `TurnState` slot (`ACTION`/`BONUS_ACTION`/`REACTION`) and, for a slot-expending cast, that no other leveled spell has already been cast this turn; `commit_casting_economy` marks both only after the cast has fully succeeded (slot-consumption included), matching the validate-before-consume pattern from ACT-05. Casting times outside the three combat slots (`ONE_HOUR`, etc.) are exempt from both checks entirely, not rejected — a 1-hour ritual (Raise Dead, Resurrection, True Resurrection) can't be "this turn's action" in any representation, so the engine treats it as a downtime cast the caller is responsible for gating contextually; dm-api's combat endpoint does exactly that with its own `_reject_uncombat_casting_time` 409 (an endpoint-appropriateness check, distinct from the engine's mechanical bookkeeping). TWF Light validation (ACT-04) remains open and is blocked on Workstream C — see below.
...
- Spell `casting_time` ignored — bonus/reaction spells consume the Action (`_actions.py:133` [SPL-03], **major**) ✅ done
...
- Two-weapon fighting never validates the Light property or a prior Attack action — any weapon works off-hand (`_actions.py:122` [ACT-04], **major**) — still open; genuinely blocked on Workstream C, since `AttackDetails.properties` is never populated by the real dm-api attack pipeline yet, so validating against it today would reject every off-hand attack
...
3. ✅ Thread spell `casting_time` into slot selection so a cast charges the correct `TurnState` slot; a `spell_slot_expended_this_turn` flag on `TurnState` rejects a second leveled-spell cast per turn (cantrips and ritual casts exempt).
...
5b. Validate two-weapon fighting: an `is_offhand` attack requires both weapons to have the Light property (from the Workstream C bridge) and a prior Attack action this turn; reject otherwise. **Still open** — needs Workstream C first.
...
**Tests**: ✅ level-5 fighter makes 2 attacks then is rejected on the 3rd; ✅ an attack behind total cover leaves `action_used=False` and a follow-up legal attack succeeds; ✅ Nick off-hand attack leaves `bonus_action_used=False`; ✅ opportunity attack consumes reaction and a second in the same round is rejected (engine tests in `test_attacks_2024.py::TestOpportunityAttacks`, API tests in `dm-api/tests/test_combat_actions.py`); ✅ a disengaged mover provokes no opportunity attack and spends no reaction; ✅ Ready stores a trigger+attack, `READIED_ACTION` fires it once via the reaction, and an unused readied action is lost at the start of the readier's own next turn (`test_attacks_2024.py::TestReadiedActions`); ✅ Healing Word (bonus) leaves the action free and a second leveled spell same turn is rejected while a cantrip is allowed (`test_spellcasting.py::TestCastingEconomy`, `dm-api/tests/test_combat_spells.py::test_second_leveled_spell_same_turn_rejected`); ✅ a downtime casting time (Raise Dead, 1 hour) is exempt from the turn-economy gate at the engine level but still 409s via the combat endpoint (`test_downtime_casting_time_exempt_from_turn_economy`, `test_downtime_casting_time_rejected_by_combat_endpoint`). Still open: an off-hand greatsword attack (non-Light) is rejected, as is an off-hand attack with no prior Attack action (ACT-04).

**Size**: L. Depends on Workstream A (shared `TurnState` refactor). This is the largest single workstream; split into B1 ✅ (Extra Attack + validation ordering + Nick — done), B2 ✅ (reactions/opportunity/Ready — done), B3 ✅ SPL-03/SPL-06 done (spell casting-time economy + one-slot-per-turn); ACT-04 (TWF Light validation) carved out separately since it's blocked on Workstream C.
...
2. **Workstream B** (action economy: Extra Attack, reactions, casting-time, one-slot-per-turn, validation ordering) — depends on A. B1 (Extra Attack + validation ordering), B2 (reactions/opportunity attacks/Ready), and B3's SPL-03/SPL-06 (spell casting-time economy + one-slot-per-turn) ✅ all done; B3's TWF Light validation (ACT-04) remains, blocked on Workstream C.

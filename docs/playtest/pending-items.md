...
`PT-<n>` is a simple incrementing id — next id is **PT-20**.

---

## Open

### PT-19 — Duplicate NPC entities when the AI re-introduces an already-established character
- **Status:** open
...
### PT-18 — Proposal narration commits an entity as fact before the DM can accept/reject it
- **Status:** open
...
### PT-17 — Illegal combat stats (negative HP, AC 0) are silently accepted on NPC/Monster creation
- **Status:** open
...
### PT-16 — Character creation wizard skips required class/species sub-choices (spells, Elf lineage, Keen Senses)
- **Status:** open
...
- **Notes:** This blocked exercising "one spell (attack-roll and/or save)" in
  Phase 6 even before the combat-action bug (PT-15) made it moot — Maret had
...
### PT-15 — Combat actions produce no visible feedback and Attack has no target-selection UI
- **Status:** open
...
- **Notes:** Three compounding issues, likely all need fixing together: (1)
  ...
  works now, but no attack can ever land.

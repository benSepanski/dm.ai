...
| Conditions (all 15) + mechanical effects | `core.conditions` | 🟡 (Stunned speed, Petrified immunity, Unconscious→Prone, duplicate can_act/speed_zero source fixed in Workstream I1 ✅; condition-immunity is now honored on every inflicting path — spell riders, Topple, unarmed grapple/shove — not just `engine.apply_condition`, Workstream I2 ✅; `apply_condition(EXHAUSTION)` now stacks `exhaustion_level` via `gain_exhaustion` instead of no-op'ing, Workstream I4's EFF-03 slice ✅; see [engine-correctness-remediation.md](engine-correctness-remediation.md); Charmed/Deafened/Grappled source-identity and repeat-save/auto-fail hooks remain in I3 and the rest of I4) |
...
| Weapon properties (incl. range, ammunition) | `data.weapons` | ✅ Heavy/Versatile/Finesse consumed via the registry bridge (Workstream C); Ammunition spends a matching `data.gear` inventory item per attack and blocks the attack when none is left, Loading caps a weapon to one main-hand shot per turn (EQP-08, `_attacks`/`_actions`). Reach/Thrown are positional/targeting concerns out of this engine's theater-of-mind scope |
| Full armor table + AC computation (dex caps, str minimums) | `data.armor` + `character_builder` + `_equipment` | ✅ armor table + AC (dex caps) ✅; worn-armor identity stored with equip/unequip AC recompute and a shield-as-body guard (Workstream D1); Str-minimum speed penalty, Stealth disadvantage from noisy armor, and the armor-training disadvantage/can't-cast gate all consumed (Workstream D2) |

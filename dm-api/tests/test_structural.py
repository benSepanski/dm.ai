...
def test_no_source_file_exceeds_400_loc() -> None:
    """Every dm-api source file must be ≤ 400 lines or carry an explicit annotation.

    Route modules that legitimately exceed 400 lines must include a
    ``# NOTE: exceeds 400 LoC`` comment in their first five lines to opt out
    of this check.
    """
    violations: list[str] = []
    ...
    assert not violations, (
        f"The following dm-api source files exceed {_MAX_LINES} lines without an "

...
def _create_enums() -> None:
    ...
    op.execute(
        "CREATE TYPE proposal_type AS ENUM ('location', 'character', 'dungeon', 'dialogue', 'combat_action')"
    )
    op.execute(
        "CREATE TYPE proposal_status AS ENUM ('pending', 'accepted', 'rejected', 'modified')"
    )
...
def _create_characters() -> None:
    op.create_table(
        ...
        sa.Column(
            "type",
            postgresql.ENUM("PC", "NPC", "MONSTER", name="character_type", create_type=False),
            nullable=False,
        ),
        sa.Column("name", sa.String(255), nullable=False),
...
def _create_sessions() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(50),
            nullable=False,
            server_default=sa.text("'dnd_5_5e'"),
        ),
...
def _create_chat_messages() -> None:
    op.create_table(
        ...
        sa.Column(
            "role",
            postgresql.ENUM("dm", "ai", "system", name="chat_role", create_type=False),
            nullable=False,
        ),
